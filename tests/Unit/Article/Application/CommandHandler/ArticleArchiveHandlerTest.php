...
use Micro\Article\Application\CommandHandler\ArticleArchiveHandler;
...
use Tests\Unit\DataProvider\Article\Application\CommandHandler\ArticleArchiveHandlerDataProvider;
use Tests\Unit\Mock\Article\Domain\Repository\ArticleRepositoryMockTrait;

/**
 * Unit tests for ArticleArchiveHandler.
 */
#[CoversClass(ArticleArchiveHandler::class)]
final class ArticleArchiveHandlerTest extends TestCase
{
    use MockeryPHPUnitIntegration;
    use ArticleRepositoryMockTrait;

    private ArticleArchiveHandler $handler;
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new ArticleArchiveHandler($this->articleRepositoryMock);
    }
    ...
    #[Test]
    #[DataProviderExternal(ArticleArchiveHandlerDataProvider::class, 'provideSuccessScenarios')]
    public function handleShouldArchiveArticleEntity(array $mockArgs, array $mockTimes, string $expectedUuid): void
    {
        // Arrange
        $processUuid = ProcessUuid::fromNative($mockArgs['processUuid']);
        $entityUuid = Uuid::fromNative($mockArgs['entityUuid']);
        $command = new ArticleArchiveCommand($processUuid, $entityUuid);

        // Create mock entity using trait helper
        $articleEntityMock = $this->createArticleEntityMock($expectedUuid);
        $articleEntityMock->shouldReceive('articleArchive')
            ->with(\Mockery::type(ProcessUuid::class))
            ->times($mockTimes['articleArchive']);
    ...
    #[Test]
    #[DataProviderExternal(ArticleArchiveHandlerDataProvider::class, 'provideFailureScenarios')]
    ...
    public function handleShouldReturnUuidString(): void
    {
        ...
        // Create mock entity using trait helper
        $articleEntityMock = $this->createArticleEntityMock($expectedUuid);
        $articleEntityMock->shouldReceive('articleArchive')->once();
