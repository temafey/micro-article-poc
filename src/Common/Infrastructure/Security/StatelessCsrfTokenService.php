 ...
 * stateless CSRF tokens in API contexts. Uses Symfony 8's
 * cryptographically signed tokens that don't require sessions.
 *
 * Usage in controllers:
 *   - Generate token: $this->csrfService->generateToken('api_mutate')
 *   - Validate: $this->csrfService->isValid('api_mutate', $token)
 *
 * Token IDs:
 *   - api_mutate: For POST/PUT/PATCH/DELETE operations
 *   - api_command: For CQRS command submissions
 *   - webhook: For webhook callback validation
 */
