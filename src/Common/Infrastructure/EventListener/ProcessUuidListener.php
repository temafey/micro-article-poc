...
final readonly class ProcessUuidListener implements EventSubscriberInterface
{
     ...
     *
     * Execution order:
     * 1. Check query parameters (?process_uuid=xxx or ?processUuid=xxx)
     * 2. Check POST parameters (for form submissions)
     * 3. Check JSON body (for Content-Type: application/json requests)
     * 4. Generate new UUID if none found
     * 5. Store in request attributes and merge into request data
     */
     ...
     *
     * Decodes raw body and looks for process_uuid or processUuid fields.
     */
    private function extractFromJsonBody($request): ?string
    {
        ...
        $this->logger?->error('DEBUG: Raw content received', [
            'content' => substr((string) $content, 0, 500),
     ...
     *
     * For JSON requests, Symfony's MapRequestPayload deserializes from $request->request ParameterBag.
     * This method decodes the JSON body, merges the process_uuid, and replaces the ParameterBag
     * so that MapRequestPayload can properly deserialize DTOs with the process_uuid field.
     *
     * @param mixed  $request       The HTTP request
     * @param string $validatedUuid The validated process UUID
     */
    private function mergeIntoJsonRequest(Request $request, string $validatedUuid): void
    {
        ...
        // Add or update process_uuid in the data array (using snake_case as per ArticleDto constant)
        $data[self::PARAM_CAMEL_CASE] = $validatedUuid;

        // Replace the request ParameterBag with merged data
        // This ensures MapRequestPayload can deserialize the complete DTO including process_uuid
