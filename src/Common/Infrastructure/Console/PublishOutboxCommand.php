...
final class PublishOutboxCommand extends Command
{
            ...
            try {
                ...
                foreach ($messages as $message) {
                    try {
                        ...
                        // Mark as published
                        $this->outboxRepository->markAsPublished(
                            [$message->getId()],
                            new DateTimeImmutable()
                        );

                        // Record successful publish metrics
                        $this->metrics->recordMessagePublished(
    ...
    private function parseMessageType(string $type): OutboxMessageType
    {
        return match ($type) {
            ...
            default => throw new \InvalidArgumentException(
                sprintf('Invalid message type: %s. Expected: event, task, all', $type)
            ),
        };
    }
    ...
    private function updatePendingCountMetrics(): void
    {
        try {
            ...
            $this->metrics->setPendingCount(
                OutboxMessageType::EVENT,
                $metrics['total_events']
            );

            // Update pending count for tasks
            $this->metrics->setPendingCount(
                OutboxMessageType::TASK,
                $metrics['total_tasks']
            );
        } catch (\Throwable $e) {
    ...
    private function classifyError(\Throwable $e): string
    {
        ...
        return match (true) {
            ...
            str_contains($className, 'AMQPException') || str_contains($className, 'AMQPChannelException') => 'amqp_error',
