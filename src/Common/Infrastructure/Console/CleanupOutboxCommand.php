...
#[AsCommand(
    ...
    description: 'Clean up old published outbox messages',
)]
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        try {
            if ($dryRun) {
                ...
                $io->success(sprintf(
                    '[DRY-RUN] Would delete %d records',
                    $countToDelete
                ));
            ...
            $deletedThisBatch = 0;
            do {
                $deletedThisBatch = $this->outboxRepository->deletePublishedBefore(
                    $cutoffDate,
                    $batchSize
                );
