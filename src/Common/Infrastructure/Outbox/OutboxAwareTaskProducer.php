...
final class OutboxAwareTaskProducer implements OutboxAwareProducerInterface
{
     ...
     *
     * @throws InvalidArgumentException If needReply is true (not supported with outbox)
     * @throws JsonException If payload serialization fails
     */
    public function sendCommand(string $command, $message, bool $needReply = false): ?Promise
    {
        ...
        if ($needReply) {
            throw new InvalidArgumentException(
                'Reply-based commands are not supported with outbox pattern. ' .
                'Use async messaging with correlation IDs instead.',
            );
        }
     ...
     *
     * Example: 'article.create.command' -> 'Article'
    ...
    private function extractAggregateType(string $commandType): string
    {
        ...
        if (count($parts) >= 1 && $parts[0] !== '') {
            return ucfirst($parts[0]); // 'article' -> 'Article'
        }
