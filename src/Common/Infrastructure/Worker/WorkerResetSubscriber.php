...
namespace Micro\Component\Common\Infrastructure\Worker;
...
final readonly class WorkerResetSubscriber implements EventSubscriberInterface
{
    ...
    {
        return [
            // Run at lowest priority to ensure all other terminate listeners complete first
            KernelEvents::TERMINATE => ['onKernelTerminate', -1024],
        ];
    }

    /**
     * Reset stateful services after request processing is complete.
     */
    public function onKernelTerminate(TerminateEvent $event): void
    {
        // Only reset on main requests, not sub-requests
    ...
    /**
     * Clear Doctrine EntityManager to prevent memory leaks.
     *
     * In worker mode, entities loaded in one request would persist in memory
     * and potentially cause:
     * - Memory growth over time
     * - Stale entity data
     * - Unit of Work accumulation
     */
    private function resetDoctrine(): void
    {
        ...
        foreach ($this->doctrine->getManagers() as $name => $manager) {
            if ($manager instanceof EntityManagerInterface) {
                // Clear identity map and detach all entities
                $manager->clear();

                // Close and reconnect if connection is stale
    ...
    /**
     * Reset all services implementing ResetInterface.
     *
     * This allows custom services to clean up worker-specific state
     * by implementing the ResetInterface.
     */
