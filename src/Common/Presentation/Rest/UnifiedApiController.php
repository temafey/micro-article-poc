...
use Micro\Component\Common\Infrastructure\Api\ApiVersionManager;
use Micro\Component\Common\Infrastructure\Api\VersionContext;
use Micro\Component\Common\Infrastructure\Api\VersionedResponseBuilder;
use Micro\Component\Common\Infrastructure\Exception\ApiVersionException;
use Micro\Component\Common\Presentation\Rest\Traits\PaginationTrait;
use Micro\Component\Common\Presentation\Rest\Traits\ResourceLinksTrait;
...
abstract class UnifiedApiController extends AbstractController
{
    ...
    /**
     * Generate task resource links with version awareness.
     *
     * @param Request $request Current request
     ...
     */
    protected function getVersionAwareTaskLinks(Request $request, string $uuid): array
    {
        $context = $this->getVersionContext($request);
        $baseUrl = $context->getVersionedBaseUrl();

        $links = [
            'self' => sprintf('%s/tasks/%s', $baseUrl, $uuid),
        ];

        // Add version-specific links
        if ($context->supportsFeature('status_updates')) {
            $links['update'] = sprintf('%s/tasks/%s/status', $baseUrl, $uuid);
        }

        if ($context->supportsFeature('task_assignment')) {
            $links['assign'] = sprintf('%s/tasks/%s/assign', $baseUrl, $uuid);
        }

        if ($context->supportsFeature('priority_levels')) {
            $links['priority'] = sprintf('%s/tasks/%s/priority', $baseUrl, $uuid);
        }

        if ($context->supportsFeature('due_dates')) {
            $links['dueDate'] = sprintf('%s/tasks/%s/due-date', $baseUrl, $uuid);
        }
    ...
    /**
     * Generate user resource links with version awareness.
     *
     * @param Request $request Current request
     ...
     * @return array<string, string> Resource links
     */
    protected function getVersionAwareUserLinks(Request $request, string $uuid): array
    {
        $context = $this->getVersionContext($request);
        $baseUrl = $context->getVersionedBaseUrl();

        return [
            'self' => sprintf('%s/users/%s', $baseUrl, $uuid),
            'tasks' => sprintf('%s/tasks?assigneeId=%s', $baseUrl, $uuid),
            'update' => sprintf('%s/users/%s', $baseUrl, $uuid),
        ];
    }
